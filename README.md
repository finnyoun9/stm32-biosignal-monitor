...
> **一页作品集摘要（给 HR / 面试官）：[PORTFOLIO.md](PORTFOLIO.md)**
>
...
| **P1 采集链路真机** | MAX30102 驱动 + FreeRTOS 任务 + 串口协议，上位机实时绘图 | 实机波形截图、采集丢包率、`docs/04` 调试记录（≥3 条问题闭环） | 🔄 **固件已可编译**（`pio run`：RAM 49.1% / Flash 32.8%，CI 持续验证）＋工具链/检查单就绪；⏳ 待 Finn 接线实测 |
...
| **固件可编译性** | `pio run` 通过（ARM GCC）：RAM **49.1%**（10,052 B / 20 KB）、Flash **32.8%**（21,496 B / 64 KB）；CI 的 `firmware-build` 任务每次 push 重新验证 | `cd firmware && pio run` |
...
| CI | GitHub Actions 每次 push 自动跑 11 组离线验证 + 1 个固件编译任务（算法/协议/驱动/PPG/功耗/ECG/双通道/体检报告），多次 success | [actions](https://github.com/finnyoun9/stm32-biosignal-monitor/actions) |
...
```
docs/        调研、范围与验收标准、硬件接线、信号处理笔记、调试记录模板、低功耗设计、ECG 通道设计、上机检查单、实测回填表
...
./.venv/bin/python tools/inspect_capture.py --selftest                 # 采集体检报告自检（好坏数据区分）
