...
#   9. 真实数据验证（PhysioNet BIDMC，需要 data/raw/ 有数据） tools/validate_bidmc.py
set -euo pipefail
...
echo "== 1/9 算法单测（Python，合成信号） =="
"$PY" tests/test_algorithms.py

echo
echo "== 2/9 C/Python 协议一致性（四类帧） =="
"$PY" tests/test_protocol_conformance.py

echo
echo "== 3/9 MAX30102 驱动单测（模拟芯片） =="
"$PY" tests/test_driver_host.py

echo
echo "== 4/9 C/Python PPG 算法一致性（固件侧实现） =="
"$PY" tests/test_algo_conformance.py

echo
echo "== 5/9 C/Python 低功耗预算一致性 =="
"$PY" tests/test_power_budget.py

echo
echo "== 6/9 C/Python ECG QRS 一致性（固件侧实现） =="
"$PY" tests/test_ecg_conformance.py

echo
echo "== 7/9 双通道采集自检 + P3 一致性验收 =="
